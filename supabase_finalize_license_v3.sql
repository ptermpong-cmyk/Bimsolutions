-- Compatibility migration for the existing subscriptions table.
...
begin
  ...
  return json_build_object('valid', true, 'plan', active_subscription.plan, 'expires_at', active_subscription.expires_at);
end;
$$;
...
begin
  select id into administrator_id from auth.users where lower(email) = lower('p.termpong@gmail.com') limit 1;
  if administrator_id is null then raise exception 'Administrator account was not found.'; end if;
  if not exists (select 1 from public.subscriptions where user_id = administrator_id and is_active = true and expires_at is null) then
